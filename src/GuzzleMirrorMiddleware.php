    ...
    public function __construct(array $options = [])
    {
        ...
        foreach ($this->options['mirrors'] as $index => $mirror) {
            $this->options['mirrors'][$index] = array_replace(
                [
                    'ignore_mirror_failures' => $this->options['ignore_mirror_failures']
                        ?? $this->options['default_mirror_options']['ignore_mirror_failures'],
                    'ignore_mirror_methods'  => $this->options['ignore_mirror_methods']
                        ?? $this->options['default_mirror_options']['ignore_mirror_methods'],
                ],
                $mirror
            );
        }
        ...
        return function (ResponseInterface $response) use ($request, $options, $middlewareOptions) {
            ...
            $promises = function () use ($request, $options, $middlewareOptions, &$ignoreFailures) {
                foreach (array_filter(
                    $middlewareOptions['mirrors'],
                    function ($mirror) use ($request, $middlewareOptions) {
                        if (in_array(
                            $request->getMethod(),
            ...
            } elseif (count($failures) && $middlewareOptions['ignored_failures_callback'] != null) {
